...
message = '''d = {
    ...
    'Inventory': dict((str(i),i) for i in iter(range(60))), 'CurrentLocation': """
        Pentos is a large port city, more populous than Astapor on Slaver Bay, and may be one of the most populous of the Free Cities.
        It lies on the bay of Pentos off the narrow sea, with the Flatlands plains and Velvet Hills to the east.
        The city has many square brick towers, controlled by the spice traders. Most of the roofing is done in tiles. There is a large red temple in Pentos, along with the manse of Illyrio Mopatis and the Sunrise Gate allows the traveler to exit the city to the east,
...
tests = [
    ...
    ('yaml', 'import yaml; %s' % setup_yaml, 'yaml.dump(d)', 'yaml.full_load(src)'),
    ('message pack', 'import msgpack; %s' % setup_msgpack, 'msgpack.packb(d)', 'msgpack.unpackb(src)')]
